...
public class StaticServiceImpl {
    private static final Class<?> PARENT = HorainingyouBotApplication.class;

    public List<String> getFilenames(String path) {
        List<String> filenames = new ArrayList<>();
        try {
            URL dirURL = PARENT.getResource("/static/" + path);
    ...
    public String getAsText(String path) {
        try (InputStream in = PARENT.getResourceAsStream("/static/" + path); BufferedReader reader = new BufferedReader(new InputStreamReader(in, StandardCharsets.UTF_8))) {
    ...
    public byte[] getAsByteArray(String path) {
        try (InputStream in = PARENT.getResourceAsStream("/static/" + path)) {
