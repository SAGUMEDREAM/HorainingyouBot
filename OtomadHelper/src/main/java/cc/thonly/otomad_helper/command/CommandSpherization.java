...
@Command
public class CommandSpherization implements CommandEntrypoint {
    @Override
    public void registerCommand(Commands commands) {
        commands.registerCommand(
                CommandNode.createRoot("球面化")
                        .withExecutor((bot, event, args) -> {
                            LinkedMessage.start(bot, event, ctx -> {
                                ...
                                MsgTool.reply(bot, event, ArrayMsgUtils.builder().img(ImageUtils.fisheye(image, 1.2)).build());
                            });
                        })
        );
