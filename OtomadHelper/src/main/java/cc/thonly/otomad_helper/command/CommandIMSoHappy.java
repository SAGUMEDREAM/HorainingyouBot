...
@Command
public class CommandIMSoHappy implements CommandEntrypoint {
    @Override
    public void registerCommand(Commands commands) {
        commands.registerCommand(
                CommandNode.createRoot("我巨爽")
                        .withExecutor((bot, event, args) -> {
                            LinkedMessage.start(bot, event, ctx -> {
                                ...
                                MsgTool.reply(bot, event,
                                        ArrayMsgUtils.builder().img(ImageUtils.imsoHappy(image, false))
                                                .img(ImageUtils.imsoHappy(image, true))
                                                .build()
                                );
                            });
                        })
