...
@Command
public class CommandMaiFriend implements CommandEntrypoint {
    @Override
    public void registerCommand(Commands commands) {
        commands.registerCommand(
                CommandNode.createRoot("旅行伙伴加入")
                        .withExecutor((bot, event, args) -> {
                            LinkedMessage.start(bot, event, ctx -> {
                                ...
                                byte[] bytes = ImageUtils.generateAvatar(OtomadHelper.class, "/static/assets/mai_join.png", image);
