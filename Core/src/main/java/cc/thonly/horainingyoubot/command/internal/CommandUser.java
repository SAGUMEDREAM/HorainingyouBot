    ...
    public void registerCommand(Commands commands) {
        commands.registerCommand(
                CommandNode.createRoot("user")
                        ...
                        .withPermissionLevel(PermissionLevel.MODERATOR)
                        .up()
                        .addNode("update-all", (bot, event, args) -> {
                            List<User> users = this.userRepository.findAll();
                            MsgTool.reply(bot, event, "更新中...");
                            Set<User> changed = new LinkedHashSet<>(16);
                            for (User user : users) {
                                Long userId = user.getUserId();
                                ActionData<StrangerInfoResp> strangerInfo = bot.getStrangerInfo(userId, false);
                                StrangerInfoResp data = strangerInfo.getData();
                                if (!Objects.equals(user.getUsername(), data.getNickname())) {
                                    user.setUsername(data.getNickname());
                                    changed.add(user);
                                }
                            }
                            this.userRepository.saveAll(changed);
                            MsgTool.reply(bot, event, "成功更新%s条用户数据".formatted(changed.size()));
                        })
                        .withPermissionLevel(PermissionLevel.MODERATOR)
                        .up()
                        .up()
